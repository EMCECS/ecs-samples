...
public class _18_BucketLifecycleVersioning {
    ...
    public static void main(String[] args) throws Exception {
        ...
        LifecycleRule rule = new LifecycleRule("rule-a", "", LifecycleRule.Status.Enabled);

        // set non-current version expiration to 30 days
        rule.setNoncurrentVersionExpirationDays(30);
        ...
        for (LifecycleRule lr : lc2.getRules()) {
            System.out.println(String.format("\t- rule: [id: %s, prefix: %s, status: %s, noncurrentVersionExpirationDays: %s",
                    lr.getId(),
                    lr.getPrefix(),
                    lr.getStatus(),
                    lr.getNoncurrentVersionExpirationDays()));
        }
    }
}
