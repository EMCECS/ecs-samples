...
enable_lio() {
    ...
    docker run --name enable_lio --privileged --rm --cap-add=SYS_ADMIN -v /lib/modules:/lib/modules -v /sys:/sys:rshared nautilus/init:0.1
    echo
}
...
run_kind() {
    ...
    kind create cluster --image nautilus/kind-node:v1.13.2
...
run_openshift() {
    ...
    cd -
    # Add nautilus service account to Security Context Constraint (SCC).
    # This is openshift specific permission which is required for the operator
    # to work.
    oc adm policy add-scc-to-user privileged system:serviceaccount:nautilus:nautilus-daemonset-sa
    oc adm policy add-scc-to-user privileged system:serviceaccount:nautilus:nautilus-statefulset-sa
    echo
}
...
main() {
    ...
    # This would build a container with tag nautilus/cluster-operator:test,
    ...
    if [ "$1" = "kind" ]; then
        ...
        docker save nautilus/cluster-operator:test > cluster-operator.tar
    ...
    if [ "$2" = "olm" ]; then
        ...
        set +o nounset
        source ./test-env/bin/activate
        operator-courier verify ./deploy/olm/nautilus
        deactivate
        set -o nounset
        ...
        kubectl create ns nautilus-operator
        ...
        operator-sdk test local ./test/e2e --go-test-flags "-v -tags intree" --namespace nautilus-operator

        echo "Deleting namespace nautilus..."
        kubectl delete ns nautilus --ignore-not-found=true

        operator-sdk test local ./test/e2e --go-test-flags "-v -tags csi" --namespace nautilus-operator

        echo "Deleting namespace nautilus..."
        kubectl delete ns nautilus --ignore-not-found=true

        # echo "**** Resource details for nautilus-operator namespace ****"
        # print_pod_details_and_logs nautilus-operator

        # echo "**** Resource details for nautilus namespace ****"
        # print_pod_details_and_logs nautilus
    fi
