...
func newReconciler(mgr manager.Manager) reconcile.Reconciler {
	...
	return &ReconcileJob{client: mgr.GetClient(), scheme: mgr.GetScheme(), clientset: clientset, recorder: mgr.GetRecorder("nautiluscluster-operator")}
}
...
func add(mgr manager.Manager, r reconcile.Reconciler) error {
	...
	err = c.Watch(&source.Kind{Type: &nautilusv1.Job{}}, &handler.EnqueueRequestForObject{})
	if err != nil {
		return err
	}
...
func (r *ReconcileJob) Reconcile(request reconcile.Request) (reconcile.Result, error) {
	...
	if err != nil && errors.IsNotFound(err) {
		log.Printf("Creating a new DaemonSet %s/%s\n", daemonset.Namespace, daemonset.Name)
...
func checkPods(client kubernetes.Interface, cr *nautilusv1.Job, recorder record.EventRecorder) (bool, error) {
	...
	if totalPods == 0 {
		log.Println("found no daemonsets")
		return false, nil
	}
	...
	for _, p := range pods.Items {
		...
		if err != nil {
			log.Printf("failed to get logs from pod %s: %v", p.GetName(), err)
...
func newDaemonSetForCR(cr *nautilusv1.Job) (*appsv1.DaemonSet, error) {
