...
import (
	...
	nautilusapis "github.com/nautilus/cluster-operator/pkg/apis"
	api "github.com/nautilus/cluster-operator/pkg/apis/nautilus/v1"
)
...
func testSetup() {
	...
	nautilusapis.AddToScheme(testScheme)
}
...
func TestCreateNamespace(t *testing.T) {
	...
	// Fetch the created namespace and check if it's a child of NautilusCluster.
...
func TestCreateDaemonSet(t *testing.T) {
	...
	for _, tc := range testcases {
		...
		stosCluster.Spec = api.NautilusClusterSpec{}
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if err := c.Create(context.Background(), stosCluster); err != nil {
				t.Fatalf("failed to create nautiluscluster object: %v", err)
			}
...
func TestDeployKVBackend(t *testing.T) {
	...
	stosCluster := &api.NautilusCluster{
		...
		Spec: api.NautilusClusterSpec{
			KVBackend: api.NautilusClusterKVBackend{
				Address: testKVAddr,
				Backend: testBackend,
			},
		},
