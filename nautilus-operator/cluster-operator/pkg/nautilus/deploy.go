...
const (
	...
	daemonsetName   = "nautilus-daemonset"
	statefulsetName = "nautilus-statefulset"

	tlsSecretType       = "kubernetes.io/tls"
	nautilusSecretType = "kubernetes.io/nautilus"

	intreeProvisionerName = "kubernetes.io/nautilus"
	csiProvisionerName    = "nautilus"
	...
	defaultUsername = "nautilus"
	defaultPassword = "nautilus"
)

// Deploy deploys nautilus by creating all the resources needed to run nautilus.
func (s *Deployment) Deploy() error {
	...
	status, err := s.getNautilusStatus()
	if err != nil {
		return fmt.Errorf("failed to get nautilus status: %v", err)
	}
	return s.updateNautilusStatus(status)
}
...
func labelsForDaemonSet(name string) map[string]string {
	return map[string]string{"app": appName, "nautilus_cr": name, "kind": daemonsetKind}
}

func labelsForStatefulSet(name string) map[string]string {
	return map[string]string{"app": appName, "nautilus_cr": name, "kind": statefulsetKind}
}
...
func asOwner(m *nautilusv1.NautilusCluster) metav1.OwnerReference {
	trueVar := true
