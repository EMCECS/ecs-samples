...
// CreateCluster creates a NautilusCluster CR with the desired spec
func CreateCluster(t *testing.T, f *framework.Framework, ctx *framework.TestCtx, p *api.NautilusCluster) (*api.NautilusCluster, error) {
	t.Logf("creating nautilus cluster: %s", p.Name)
	...
	nautilus := &api.NautilusCluster{}
	err = f.Client.Get(goctx.TODO(), types.NamespacedName{Namespace: p.Namespace, Name: p.Name}, nautilus)
	if err != nil {
		return nil, fmt.Errorf("failed to obtain created CR: %v", err)
	}
	t.Logf("created nautilus cluster: %s", nautilus.Name)
	return nautilus, nil
}

// DeleteCluster deletes the NautilusCluster CR specified by cluster spec
func DeleteCluster(t *testing.T, f *framework.Framework, ctx *framework.TestCtx, p *api.NautilusCluster) error {
	t.Logf("deleting nautilus cluster: %s", p.Name)
	err := f.Client.Delete(goctx.TODO(), p)
	if err != nil {
		return fmt.Errorf("failed to delete CR: %v", err)
	}

	t.Logf("deleted nautilus cluster: %s", p.Name)
	return nil
}

// UpdateCluster updates the NautilusCluster CR
func UpdateCluster(t *testing.T, f *framework.Framework, ctx *framework.TestCtx, p *api.NautilusCluster) error {
	t.Logf("updating nautilus cluster: %s", p.Name)
	err := f.Client.Update(goctx.TODO(), p)
	if err != nil {
		return fmt.Errorf("failed to update CR: %v", err)
	}

	t.Logf("updated nautilus cluster: %s", p.Name)
	return nil
}

// GetCluster returns the latest NautilusCluster CR
func GetCluster(t *testing.T, f *framework.Framework, ctx *framework.TestCtx, p *api.NautilusCluster) (*api.NautilusCluster, error) {
	nautilus := &api.NautilusCluster{}
	err := f.Client.Get(goctx.TODO(), types.NamespacedName{Namespace: p.Namespace, Name: p.Name}, nautilus)
	if err != nil {
		return nil, fmt.Errorf("failed to obtain created CR: %v", err)
...
func WaitForClusterToBecomeReady(t *testing.T, f *framework.Framework, ctx *framework.TestCtx, p *api.NautilusCluster, size int) error {
	...
	t.Logf("nautilus cluster ready: %s", p.Name)
	return nil
}

// WaitForClusterToTerminate will wait until all cluster pods are terminated
func WaitForClusterToTerminate(t *testing.T, f *framework.Framework, ctx *framework.TestCtx, p *api.NautilusCluster) error {
	t.Logf("waiting for nautilus cluster to terminate: %s", p.Name)

	listOptions := metav1.ListOptions{
		LabelSelector: labels.SelectorFromSet(util.LabelsForNautilusCluster(p)).String(),
	}
	...
	t.Logf("nautilus cluster terminated: %s", p.Name)
	return nil
}

// WriteAndReadData writes sample data and reads it back from the given Nautilus cluster
func WriteAndReadData(t *testing.T, f *framework.Framework, ctx *framework.TestCtx, p *api.NautilusCluster) error {
	t.Logf("writing and reading data from nautilus cluster: %s", p.Name)
	...
	t.Logf("nautilus cluster validated: %s", p.Name)
	return nil
}
...
func RestartTier2(t *testing.T, f *framework.Framework, ctx *framework.TestCtx, namespace string) error {
	...
	t.Logf("nautilus cluster tier2 restarted")
	return nil
}

func CheckPvcSanity(t *testing.T, f *framework.Framework, ctx *framework.TestCtx, p *api.NautilusCluster) error {
	...
	for _, pvc := range pvcList.Items {
		...
		if util.PvcIsOrphan(pvc.Name, p.Spec.Nautilus.NodeReplicas) {
