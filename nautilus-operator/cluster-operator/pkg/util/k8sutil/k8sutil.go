...
import (
	...
	"github.com/nautilus/cluster-operator/pkg/util/task"
...
func (k K8SOps) EventRecorder() record.EventRecorder {
	...
	recorder := eventBroadcaster.NewRecorder(
		scheme.Scheme,
		corev1.EventSource{Component: "nautiluscluster-operator"},
	)
	return recorder
}
...
// GetNautilusApps returns a DeploymentList and a StatefulSetList of the apps
// using Nautilus PVC.
func (k K8SOps) GetNautilusApps() (*appsv1.DeploymentList, *appsv1.StatefulSetList, error) {
	stosDeployments, err := k.GetDeploymentsUsingStorageClassProvisioner(nautilusProvisioner)
	if err != nil {
		return nil, nil, err
	}

	stosStatefulSets, err := k.GetStatefulSetsUsingStorageClassProvisioner(nautilusProvisioner)
	if err != nil {
		return nil, nil, err
	}
...
// ScaleDownApps fetches all the applications running nautilus and scales down
// their replica count to zero.
func (k K8SOps) ScaleDownApps() error {
	deps, ss, err := k.GetNautilusApps()
	if err != nil {
		return err
	}
...
// ScaleUpApps fetches all the applications running nautilus and scales up
...
// UpgradeDaemonSet upgrades the nautilus daemonsets with a new container
...
func (k K8SOps) UpgradeDaemonSet(newImage string) error {
	ds, err := k.GetNautilusDaemonSet()
	if err != nil {
		return err
	}

	log.Printf("updating nautilus daemonset: [%s] %s", ds.GetNamespace(), ds.GetName())
...
// GetNautilusDaemonSet returns a DaemonSet that runs nautilus.
func (k K8SOps) GetNautilusDaemonSet() (*appsv1.DaemonSet, error) {
	dss, err := k.GetDaemonSetsByLabel("app=nautilus")
	if err != nil {
		return nil, err
	}

	if len(dss.Items) == 0 {
		return nil, fmt.Errorf("could not find any nautilus daemonset")
	}

	if len(dss.Items) > 1 {
		return nil, fmt.Errorf("can't upgrade, found more than one nautilus daemonset")
	}
