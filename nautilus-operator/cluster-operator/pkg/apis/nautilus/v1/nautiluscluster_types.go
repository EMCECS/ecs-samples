...
// ClusterPhase is the phase of the nautilus cluster at a given point in time.
...
const (
	...
	DefaultNamespace = "nautilus"

	DefaultServiceName         = "nautilus"
	DefaultServiceType         = "ClusterIP"
	DefaultServiceExternalPort = 5705
	DefaultServiceInternalPort = 5705

	DefaultIngressHostname = "nautilus.local"

	DefaultNodeContainerImage                 = "nautilus/node:1.1.4"
	DefaultInitContainerImage                 = "nautilus/init:0.1"
	...
	CSIv1ExternalProvisionerContainerImage    = "nautilus/csi-provisioner:v1.0.1"
	...
	CSIv0ExternalProvisionerContainerImage    = "nautilus/csi-provisioner:v0.4.2"
...
// NautilusClusterSpec defines the desired state of NautilusCluster
type NautilusClusterSpec struct {
	...
	CSI NautilusClusterCSI `json:"csi"`

	// ResourceNS is the kubernetes Namespace where nautilus resources are
	// provisioned.
	ResourceNS string `json:"namespace"`

	// Service is the Service configuration for the cluster nodes.
	Service NautilusClusterService `json:"service"`
	...
	// in a container.
	// Typically: "/var/lib/kubelet/plugins/kubernetes.io~nautilus".
	// If not set, defaults will be used.
	SharedDir string `json:"sharedDir"`

	// Ingress defines the ingress configurations used in the cluster.
	Ingress NautilusClusterIngress `json:"ingress"`
	...
	KVBackend NautilusClusterKVBackend `json:"kvBackend"`
	...
	// NodeSelectorTerms is to set the placement of nautilus pods using
	...
	// Tolerations is to set the placement of nautilus pods using
	// pod toleration.
	Tolerations []corev1.Toleration `json:"tolerations"`

	// Resources is to set the resource requirements of the nautilus containers.
	...
	// on a Pod, Nautilus will enable automated Pod failover (by killing the
	// application Pod on the failed node) if the following conditions exist:
	//
	// - Pod fencing has not been explicitly disabled.
	// - Nautilus has determined that the node the Pod is running on is
	//   offline.  Nautilus uses Gossip and TCP checks and will retry for 30
	...
	// - The Pod has at least one Nautilus volume attached.
	// - Each Nautilus volume has at least 1 healthy replica.
	//
	// When Pod Fencing is disabled, Nautilus will not perform any interaction
...
// NautilusClusterStatus defines the observed state of NautilusCluster
type NautilusClusterStatus struct {
...
// NautilusCluster is the Schema for the nautilusclusters API
// +k8s:openapi-gen=true
type NautilusCluster struct {
	metav1.TypeMeta   `json:",inline"`
	metav1.ObjectMeta `json:"metadata,omitempty"`

	Spec   NautilusClusterSpec   `json:"spec,omitempty"`
	Status NautilusClusterStatus `json:"status,omitempty"`
}

// +k8s:deepcopy-gen:interfaces=k8s.io/apimachinery/pkg/runtime.Object

// NautilusClusterList contains a list of NautilusCluster
type NautilusClusterList struct {
	metav1.TypeMeta `json:",inline"`
	metav1.ListMeta `json:"metadata,omitempty"`
	Items           []NautilusCluster `json:"items"`
}

func init() {
	SchemeBuilder.Register(&NautilusCluster{}, &NautilusClusterList{})
}

// MembersStatus stores the status details of cluster member nodes.
type MembersStatus struct {
	// Ready are the nautilus cluster members that are ready to serve requests.
	// The member names are the same as the node IPs.
	Ready []string `json:"ready,omitempty"`
	// Unready are the nautilus cluster nodes not ready to serve requests.
	Unready []string `json:"unready,omitempty"`
}

// GetResourceNS returns the namespace where all the resources should be provisioned.
func (s NautilusClusterSpec) GetResourceNS() string {
	if s.ResourceNS != "" {
		return s.ResourceNS
	}
...
func (s NautilusClusterSpec) GetNodeContainerImage() string {
...
func (s NautilusClusterSpec) GetInitContainerImage() string {
...
func (s NautilusClusterSpec) GetCSINodeDriverRegistrarImage(csiv1 bool) string {
...
// container image.
func (s NautilusClusterSpec) GetCSIClusterDriverRegistrarImage() string {
...
func (s NautilusClusterSpec) GetCSIExternalProvisionerImage(csiv1 bool) string {
...
func (s NautilusClusterSpec) GetCSIExternalAttacherImage(csiv1 bool) string {
...
func (s NautilusClusterSpec) GetCSILivenessProbeImage() string {
...
func (s NautilusClusterSpec) GetServiceName() string {
	if s.Service.Name != "" {
...
func (s NautilusClusterSpec) GetServiceType() string {
	if s.Service.Type != "" {
...
func (s NautilusClusterSpec) GetServiceExternalPort() int {
...
func (s NautilusClusterSpec) GetServiceInternalPort() int {
...
func (s NautilusClusterSpec) GetIngressHostname() string {
...
func (s NautilusClusterSpec) GetCSIEndpoint(csiv1 bool) string {
	if s.CSI.Endpoint != "" {
...
func (s NautilusClusterSpec) GetCSIRegistrarSocketDir() string {
...
func (s NautilusClusterSpec) GetCSIKubeletDir() string {
	if s.CSI.KubeletDir != "" {
...
func (s NautilusClusterSpec) GetCSIPluginDir(csiv1 bool) string {
	if s.CSI.PluginDir != "" {
...
func (s NautilusClusterSpec) GetCSIDeviceDir() string {
	if s.CSI.DeviceDir != "" {
...
func (s NautilusClusterSpec) GetCSIRegistrationDir(csiv1 bool) string {
...
func (s NautilusClusterSpec) GetCSIKubeletRegistrationPath(csiv1 bool) string {
...
func (s NautilusClusterSpec) GetCSIDriverRegistrationMode() string {
...
func (s NautilusClusterSpec) GetCSIDriverRequiresAttachment() string {
...
func (s NautilusClusterSpec) GetCSIVersion(csiv1 bool) string {
	if s.CSI.Version != "" {
...
// NautilusClusterCSI contains CSI configurations.
type NautilusClusterCSI struct {
...
// NautilusClusterService contains Service configurations.
type NautilusClusterService struct {
...
// NautilusClusterIngress contains Ingress configurations.
type NautilusClusterIngress struct {
	Enable      bool              `json:"enable"`
...
// NautilusClusterKVBackend stores key-value store backend configurations.
type NautilusClusterKVBackend struct {
