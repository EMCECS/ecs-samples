...
import (
	...
	ecsapis "github.com/ecs/cluster-operator/pkg/apis"
	api "github.com/ecs/cluster-operator/pkg/apis/ecs/v1"
)
...
func testSetup() {
	...
	ecsapis.AddToScheme(testScheme)
}
...
func TestCreateNamespace(t *testing.T) {
	...
	// Fetch the created namespace and check if it's a child of ECSCluster.
...
func TestCreateDaemonSet(t *testing.T) {
	...
	for _, tc := range testcases {
		...
		stosCluster.Spec = api.ECSClusterSpec{}
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if err := c.Create(context.Background(), stosCluster); err != nil {
				t.Fatalf("failed to create ecscluster object: %v", err)
			}
...
func TestDeployKVBackend(t *testing.T) {
	...
	stosCluster := &api.ECSCluster{
		...
		Spec: api.ECSClusterSpec{
			KVBackend: api.ECSClusterKVBackend{
				Address: testKVAddr,
				Backend: testBackend,
			},
		},
