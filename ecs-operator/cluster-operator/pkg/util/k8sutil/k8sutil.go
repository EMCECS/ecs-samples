...
import (
	...
	"github.com/ecs/cluster-operator/pkg/util/task"
...
func (k K8SOps) EventRecorder() record.EventRecorder {
	...
	recorder := eventBroadcaster.NewRecorder(
		scheme.Scheme,
		corev1.EventSource{Component: "ecscluster-operator"},
	)
	return recorder
}
...
// GetECSApps returns a DeploymentList and a StatefulSetList of the apps
// using ECS PVC.
func (k K8SOps) GetECSApps() (*appsv1.DeploymentList, *appsv1.StatefulSetList, error) {
	stosDeployments, err := k.GetDeploymentsUsingStorageClassProvisioner(ecsProvisioner)
	if err != nil {
		return nil, nil, err
	}

	stosStatefulSets, err := k.GetStatefulSetsUsingStorageClassProvisioner(ecsProvisioner)
	if err != nil {
		return nil, nil, err
	}
...
// ScaleDownApps fetches all the applications running ecs and scales down
// their replica count to zero.
func (k K8SOps) ScaleDownApps() error {
	deps, ss, err := k.GetECSApps()
	if err != nil {
		return err
	}
...
// ScaleUpApps fetches all the applications running ecs and scales up
...
// UpgradeDaemonSet upgrades the ecs daemonsets with a new container
...
func (k K8SOps) UpgradeDaemonSet(newImage string) error {
	ds, err := k.GetECSDaemonSet()
	if err != nil {
		return err
	}

	log.Printf("updating ecs daemonset: [%s] %s", ds.GetNamespace(), ds.GetName())
...
// GetECSDaemonSet returns a DaemonSet that runs ecs.
func (k K8SOps) GetECSDaemonSet() (*appsv1.DaemonSet, error) {
	dss, err := k.GetDaemonSetsByLabel("app=ecs")
	if err != nil {
		return nil, err
	}

	if len(dss.Items) == 0 {
		return nil, fmt.Errorf("could not find any ecs daemonset")
	}

	if len(dss.Items) > 1 {
		return nil, fmt.Errorf("can't upgrade, found more than one ecs daemonset")
	}
