...
// Add creates a new ECSUpgrade Controller and adds it to the Manager. The Manager will set fields on the Controller
...
func newReconciler(mgr manager.Manager) reconcile.Reconciler {
	return &ReconcileECSUpgrade{client: mgr.GetClient(), scheme: mgr.GetScheme(), recorder: mgr.GetRecorder("ecs-upgrader")}
}
...
func add(mgr manager.Manager, r reconcile.Reconciler) error {
	...
	// Watch for changes to primary resource ECSUpgrade
	err = c.Watch(&source.Kind{Type: &ecsv1.ECSUpgrade{}}, &handler.EnqueueRequestForObject{})
	if err != nil {
		return err
	}
...
var _ reconcile.Reconciler = &ReconcileECSUpgrade{}

// ReconcileECSUpgrade reconciles a ECSUpgrade object
type ReconcileECSUpgrade struct {
	// This client, initialized using mgr.Client() above, is a split client
	// that reads objects from the cache and writes to the apiserver
	client         client.Client
	scheme         *runtime.Scheme
	currentUpgrade *ecsv1.ECSUpgrade
	currentCluster *ecsv1.ECSCluster
	imagePuller    *ecsv1.Job
...
func (r *ReconcileECSUpgrade) SetCurrentUpgradeIfNone(upgrade *ecsv1.ECSUpgrade) {
...
func (r *ReconcileECSUpgrade) SetCurrentUpgrade(upgrade *ecsv1.ECSUpgrade) {
	r.currentUpgrade = upgrade
}

// IsCurrentUpgrade compares a given upgrade with the current cluster to check
// if they are the same.
func (r *ReconcileECSUpgrade) IsCurrentUpgrade(upgrade *ecsv1.ECSUpgrade) bool {
	if upgrade == nil {
		return false
	}
...
func (r *ReconcileECSUpgrade) ResetCurrentUpgrade(request reconcile.Request) {
...
func (r *ReconcileECSUpgrade) findCurrentCluster() (*ecsv1.ECSCluster, error) {
	clusterList := &ecsv1.ECSClusterList{}
	if err := r.client.List(context.TODO(), &client.ListOptions{}, clusterList); err != nil {
		return nil, fmt.Errorf("failed to list clusters: %v", err)
	}

	var currentCluster *ecsv1.ECSCluster
	for _, cluster := range clusterList.Items {
		// The cluster with Phase "Running" is the only active cluster.
		if cluster.Status.Phase == ecsv1.ClusterPhaseRunning {
...
// currentCluster.
func (r *ReconcileECSUpgrade) findAndSetCurrentCluster() error {
...
func (r *ReconcileECSUpgrade) resetCurrentCluster(request reconcile.Request) {
...
func (r *ReconcileECSUpgrade) resetImagePuller(request reconcile.Request) {
...
func (r *ReconcileECSUpgrade) pauseCluster() error {
...
func (r *ReconcileECSUpgrade) resumeCluster(request reconcile.Request) error {
...
// upgrade.
func (r *ReconcileECSUpgrade) requestFromCurrentUpgrade(request reconcile.Request) bool {
...
// Reconcile reads that state of the cluster for a ECSUpgrade object and makes changes based on the state read
// and what is in the ECSUpgrade.Spec
...
func (r *ReconcileECSUpgrade) Reconcile(request reconcile.Request) (reconcile.Result, error) {
	// log.Printf("Reconciling ECSUpgrade %s/%s\n", request.Namespace, request.Name)

	reconcilePeriod := 10 * time.Second
	reconcileResult := reconcile.Result{RequeueAfter: reconcilePeriod}

	// Fetch the ECSUpgrade instance
	instance := &ecsv1.ECSUpgrade{}
	...
	if !r.IsCurrentUpgrade(instance) {
		err := fmt.Errorf("can't create more than one ecs upgrade")
	...
	sa := newServiceAccountForCR("ecs-upgrader-sa", instance)
	...
	cr := newClusterRole("ecs-upgrader-role", rules)
	...
	crb := newClusterRoleBinding("ecs-upgrader-clusterrolebinding", subjects, roleRef)
	...
	// Set ECSUpgrade instance as the owner and controller.
	...
	if err != nil && errors.IsNotFound(err) {
		...
		// process.
		upgradeInitMessage := fmt.Sprintf("ECS upgrade of cluster %s started", r.currentCluster.GetName())
	...
	if found.Status.Succeeded == 1 {
		...
		successMessage := fmt.Sprintf("ECS upgraded to %s. Delete upgrade object to disable cluster maintenance mode", instance.Spec.NewImage)
...
// GetECSClient returns a ECS client.
func (r *ReconcileECSUpgrade) GetECSClient() (*ecsapi.Client, error) {
	...
	stosCli, err := ecsapi.NewVersionedClient(strings.Join([]string{serviceInstance.Spec.ClusterIP, ecsapi.DefaultPort}, ":"), ecsapi.DefaultVersionStr)
	if err != nil {
		return nil, err
	}
...
func (r *ReconcileECSUpgrade) EnableClusterMaintenance() error {
	stosCli, err := r.GetECSClient()
	if err != nil {
		return err
	}
...
func (r *ReconcileECSUpgrade) DisableClusterMaintenance() error {
	stosCli, err := r.GetECSClient()
	if err != nil {
		return err
	}
...
func newJobForCR(cr *ecsv1.ECSUpgrade) *batchv1.Job {
	...
	return &batchv1.Job{
		...
		Spec: batchv1.JobSpec{
			Template: corev1.PodTemplateSpec{
				Spec: corev1.PodSpec{
					ServiceAccountName: "ecs-upgrader-sa",
...
func newServiceAccountForCR(name string, cr *ecsv1.ECSUpgrade) *corev1.ServiceAccount {
