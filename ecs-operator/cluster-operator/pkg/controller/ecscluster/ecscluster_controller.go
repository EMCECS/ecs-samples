...
import (
	...
	"github.com/ecs/cluster-operator/pkg/util/k8sutil"
...
func Add(mgr manager.Manager) error {
	// Get k8s version from client and set the version in ReconcileECSCluster.
...
func newReconciler(mgr manager.Manager, k8sVersion string) reconcile.Reconciler {
	return &ReconcileECSCluster{
		client: mgr.GetClient(), scheme: mgr.GetScheme(), k8sVersion: k8sVersion, recorder: mgr.GetRecorder("ecscluster-operator"),
	}
}
...
func add(mgr manager.Manager, r reconcile.Reconciler) error {
	...
	err = c.Watch(&source.Kind{Type: &ecsv1.ECSCluster{}}, &handler.EnqueueRequestForObject{})
	if err != nil {
		return err
	}
...
func (r *ReconcileECSCluster) SetCurrentClusterIfNone(cluster *ecsv1.ECSCluster) {
...
func (r *ReconcileECSCluster) SetCurrentCluster(cluster *ecsv1.ECSCluster) {
	r.currentCluster = NewECSCluster(cluster)
}

// ResetCurrentCluster resets the current cluster of the controller.
func (r *ReconcileECSCluster) ResetCurrentCluster() {
...
func (r *ReconcileECSCluster) Reconcile(request reconcile.Request) (reconcile.Result, error) {
	...
	// There must be only a single instance of ecs in a cluster.
	if !r.currentCluster.IsCurrentCluster(instance) {
		err := fmt.Errorf("can't create more than one ecs cluster")
...
func (r *ReconcileECSCluster) reconcile(m *ecsv1.ECSCluster) error {
	if m.Spec.Pause {
	...
	if m.Spec.Join != join {
		m.Spec.Join = join
		// Update Nodes as well, because updating ECS with null Nodes
	...
	if m.Spec.CSI.Enable {
		m.Spec.Images.CSINodeDriverRegistrarContainer = m.Spec.GetCSINodeDriverRegistrarImage(ecs.CSIV1Supported(r.k8sVersion))
		if ecs.CSIV1Supported((r.k8sVersion)) {
			m.Spec.Images.CSIClusterDriverRegistrarContainer = m.Spec.GetCSIClusterDriverRegistrarImage()
		}
		m.Spec.Images.CSIExternalProvisionerContainer = m.Spec.GetCSIExternalProvisionerImage(ecs.CSIV1Supported(r.k8sVersion))
		m.Spec.Images.CSIExternalAttacherContainer = m.Spec.GetCSIExternalAttacherImage(ecs.CSIV1Supported(r.k8sVersion))
	}
...
func (r *ReconcileECSCluster) generateJoinToken(m *ecsv1.ECSCluster) (string, error) {
	// Get a new list of all the nodes.
	nodeList := ecs.NodeList()
	...
	for _, node := range nodeList.Items {
		// Skip nodes which have taints not tolerated by ecs.
	...
	nodeIPs := ecs.GetNodeIPs(selectedNodes)
