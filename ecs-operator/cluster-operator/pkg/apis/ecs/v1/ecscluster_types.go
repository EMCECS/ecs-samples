...
// ClusterPhase is the phase of the ecs cluster at a given point in time.
...
const (
	...
	DefaultServiceName         = "ecs"
	DefaultServiceType         = "ClusterIP"
	DefaultServiceExternalPort = 5705
	DefaultServiceInternalPort = 5705

	DefaultIngressHostname = "ecs.local"

	DefaultNodeContainerImage                 = "ecs/node:1.1.4"
	DefaultInitContainerImage                 = "ecs/init:0.1"
	...
	CSIv1ExternalProvisionerContainerImage    = "ecs/csi-provisioner:v1.0.1"
	...
	CSIv0ExternalProvisionerContainerImage    = "ecs/csi-provisioner:v0.4.2"
...
type ECSClusterSpec struct {
	...
	CSI ECSClusterCSI `json:"csi"`

	// ResourceNS is the kubernetes Namespace where ecs resources are
	// provisioned.
	ResourceNS string `json:"namespace"`

	// Service is the Service configuration for the cluster nodes.
	Service ECSClusterService `json:"service"`
	...
	// in a container.
	// Typically: "/var/lib/kubelet/plugins/kubernetes.io~ecs".
	// If not set, defaults will be used.
	SharedDir string `json:"sharedDir"`

	// Ingress defines the ingress configurations used in the cluster.
	Ingress ECSClusterIngress `json:"ingress"`
	...
	KVBackend ECSClusterKVBackend `json:"kvBackend"`
	...
	// NodeSelectorTerms is to set the placement of ecs pods using
	...
	// Tolerations is to set the placement of ecs pods using
	// pod toleration.
	Tolerations []corev1.Toleration `json:"tolerations"`

	// Resources is to set the resource requirements of the ecs containers.
	...
	// - ECS has determined that the node the Pod is running on is
	//   offline.  ECS uses Gossip and TCP checks and will retry for 30
	...
	//
	// When Pod Fencing is disabled, ECS will not perform any interaction
...
type MembersStatus struct {
	// Ready are the ecs cluster members that are ready to serve requests.
	// The member names are the same as the node IPs.
	Ready []string `json:"ready,omitempty"`
	// Unready are the ecs cluster nodes not ready to serve requests.
	Unready []string `json:"unready,omitempty"`
}

// GetResourceNS returns the namespace where all the resources should be provisioned.
func (s ECSClusterSpec) GetResourceNS() string {
	if s.ResourceNS != "" {
		return s.ResourceNS
	}
...
func (s ECSClusterSpec) GetNodeContainerImage() string {
...
func (s ECSClusterSpec) GetInitContainerImage() string {
...
func (s ECSClusterSpec) GetCSINodeDriverRegistrarImage(csiv1 bool) string {
...
// container image.
func (s ECSClusterSpec) GetCSIClusterDriverRegistrarImage() string {
...
func (s ECSClusterSpec) GetCSIExternalProvisionerImage(csiv1 bool) string {
...
func (s ECSClusterSpec) GetCSIExternalAttacherImage(csiv1 bool) string {
...
func (s ECSClusterSpec) GetCSILivenessProbeImage() string {
...
func (s ECSClusterSpec) GetServiceName() string {
	if s.Service.Name != "" {
...
func (s ECSClusterSpec) GetServiceType() string {
	if s.Service.Type != "" {
...
func (s ECSClusterSpec) GetServiceExternalPort() int {
...
func (s ECSClusterSpec) GetServiceInternalPort() int {
...
func (s ECSClusterSpec) GetIngressHostname() string {
...
func (s ECSClusterSpec) GetCSIEndpoint(csiv1 bool) string {
	if s.CSI.Endpoint != "" {
...
func (s ECSClusterSpec) GetCSIRegistrarSocketDir() string {
...
func (s ECSClusterSpec) GetCSIKubeletDir() string {
	if s.CSI.KubeletDir != "" {
...
func (s ECSClusterSpec) GetCSIPluginDir(csiv1 bool) string {
	if s.CSI.PluginDir != "" {
...
func (s ECSClusterSpec) GetCSIDeviceDir() string {
	if s.CSI.DeviceDir != "" {
...
func (s ECSClusterSpec) GetCSIRegistrationDir(csiv1 bool) string {
...
func (s ECSClusterSpec) GetCSIKubeletRegistrationPath(csiv1 bool) string {
...
func (s ECSClusterSpec) GetCSIDriverRegistrationMode() string {
...
func (s ECSClusterSpec) GetCSIDriverRequiresAttachment() string {
...
func (s ECSClusterSpec) GetCSIVersion(csiv1 bool) string {
	if s.CSI.Version != "" {
...
// ECSClusterCSI contains CSI configurations.
type ECSClusterCSI struct {
...
// ECSClusterService contains Service configurations.
...
// ECSClusterIngress contains Ingress configurations.
type ECSClusterIngress struct {
	Enable      bool              `json:"enable"`
...
// ECSClusterKVBackend stores key-value store backend configurations.
type ECSClusterKVBackend struct {
