...
import (
	...
	"github.com/ecs/cluster-operator/pkg/apis"
...
// NewECSCluster returns a ECSCluster object, created using a given
// cluster spec.
func NewECSCluster(namespace string, clusterSpec ecs.ECSClusterSpec) *ecs.ECSCluster {
...
func SetupOperator(t *testing.T, ctx *framework.TestCtx) {
	...
	err = e2eutil.WaitForDeployment(t, f.KubeClient, namespace, "ecs-cluster-operator", 1, RetryInterval, Timeout)
	if err != nil {
		t.Fatal(err)
	}
}
...
// number of nodes.
func ClusterStatusCheck(t *testing.T, status ecs.ECSClusterStatus, nodes int) {
	if len(status.Nodes) != nodes {
		t.Errorf("unexpected number of nodes:\n\t(GOT) %d\n\t(WNT) %d", len(status.Nodes), nodes)
	}

	if status.Phase != ecs.ClusterPhaseRunning {
		t.Errorf("unexpected cluster phase:\n\t(GOT) %s\n\t(WNT) %s", status.Phase, ecs.ClusterPhaseRunning)
	}
...
// ecs daemonset is deployed successfully.
func DeployCluster(t *testing.T, ctx *framework.TestCtx, cluster *ecs.ECSCluster) error {
	f := framework.Global
	...
	clusterSecret := &corev1.Secret{
		...
		Type: corev1.SecretType("kubernetes.io/ecs"),
	...
	err = WaitForDaemonSet(t, f.KubeClient, cluster.Spec.GetResourceNS(), "ecs-daemonset", RetryInterval, Timeout*2)
	if err != nil {
		t.Fatal(err)
	}

	if cluster.Spec.CSI.Enable {
		err = WaitForStatefulSet(t, f.KubeClient, cluster.Spec.GetResourceNS(), "ecs-statefulset", RetryInterval, Timeout*2)
		if err != nil {
			t.Fatal(err)
		}
	}
