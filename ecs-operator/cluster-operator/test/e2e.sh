...
enable_lio() {
    ...
    docker run --name enable_lio --privileged --rm --cap-add=SYS_ADMIN -v /lib/modules:/lib/modules -v /sys:/sys:rshared ecs/init:0.1
    echo
}
...
run_kind() {
    ...
    kind create cluster --image ecs/kind-node:v1.13.2
...
run_openshift() {
    ...
    cd -
    # Add ecs service account to Security Context Constraint (SCC).
    # This is openshift specific permission which is required for the operator
    # to work.
    oc adm policy add-scc-to-user privileged system:serviceaccount:ecs:ecs-daemonset-sa
    oc adm policy add-scc-to-user privileged system:serviceaccount:ecs:ecs-statefulset-sa
    echo
}
...
main() {
    ...
    # This would build a container with tag ecs/cluster-operator:test,
    ...
    if [ "$1" = "kind" ]; then
        ...
        docker save ecs/cluster-operator:test > cluster-operator.tar
    ...
    if [ "$2" = "olm" ]; then
        ...
        set +o nounset
        source ./test-env/bin/activate
        operator-courier verify ./deploy/olm/ecs
        deactivate
        set -o nounset
        ...
        kubectl create ns ecs-operator
        ...
        operator-sdk test local ./test/e2e --go-test-flags "-v -tags intree" --namespace ecs-operator

        echo "Deleting namespace ecs..."
        kubectl delete ns ecs --ignore-not-found=true

        operator-sdk test local ./test/e2e --go-test-flags "-v -tags csi" --namespace ecs-operator

        echo "Deleting namespace ecs..."
        kubectl delete ns ecs --ignore-not-found=true

        # echo "**** Resource details for ecs-operator namespace ****"
        # print_pod_details_and_logs ecs-operator

        # echo "**** Resource details for ecs namespace ****"
        # print_pod_details_and_logs ecs
    fi
